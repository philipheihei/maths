...
with open('src/apps/DispersionQuiz.jsx', 'r', encoding='utf-8') as f:
    text = f.read()
...
const generateLV2CumulativeFreq = () => {
  for (let attempt = 0; attempt < 300; attempt++) {
    ...
    return {
      ...
      context: 下表顯示某班學生的頻數分佈表及累積頻數分佈表,
      xLabelFreq: ${ctx.label}（）,
      xLabelCf: ${ctx.label}少於（）,
      limits,
      lessThans,
      freqDisplays,
      cfDisplays,
...
text = text.replace('// 題型生成器陣列 — 新增題型時只需在此加入新函數\nconst LV2_GENERATORS = [\n', new_gen_func)

new_comp = '''// LV2 累積頻數表顯示元件
const LV2CumulativeFreqTableDisplay = ({ xLabelFreq, xLabelCf, limits, lessThans, freqDisplays, cfDisplays }) => (
  <div className="flex flex-col sm:flex-row gap-4 my-3 justify-center items-start">
    <div className="overflow-x-auto border border-gray-300 shadow-sm rounded-lg flex-1 w-full sm:w-auto">
      <table className="border-collapse bg-white text-sm sm:text-base w-full">
        <thead>
          <tr>
            ...
            <th className="border border-slate-400 px-4 py-2 font-bold text-center bg-blue-50 text-blue-800 break-keep">
              頻數
            </th>
          </tr>
        </thead>
      ...
      <table className="border-collapse bg-white text-sm sm:text-base w-full">
        <thead>
          <tr>
            ...
            <th className="border border-slate-400 px-4 py-2 font-bold text-center bg-emerald-50 text-emerald-800 break-keep">
              累積頻數
            </th>
          </tr>
        </thead>
...
text = text.replace('// LV2 頻數表顯示元件', new_comp)

display_code = '''          <div className="bg-white rounded-xl shadow-lg p-5 mb-4">
...
text = text.replace('<div className="bg-white rounded-xl shadow-lg p-5 mb-4">\n            <p className="text-slate-700 font-medium mb-1">{lv2Q.context}。</p>\n            {lv2Q.type === \'stem-leaf-unknown\' ? (', display_code)

with open('src/apps/DispersionQuiz.jsx', 'w', encoding='utf-8') as f:
    f.write(text)
