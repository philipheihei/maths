    ...
    <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-4 mb-6">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <button
          onClick={onDelete}
          onMouseDown={(e) => e.preventDefault()}
          className="py-3 bg-red-100 hover:bg-red-200 text-red-700 font-bold rounded-lg transition act          const TwoLineNumberLine = ({ min, max, lines, highlightRegion }) => {
            const width = 400;
            const height = 140;
            const padding = 40;
            const usableWidth = width - 2 * padding;
                ...
                {highlightRegion && (
                  <>
                    {highlightRegion.regions ? (
                      // 多個分離區域（用於OR分離情況）
                      highlightRegion.regions.map((region, idx) => (
                        <rect
                          key={`highlight-${idx}`}
                          x={getX(region.start)}
          ...
          };          // 修改第4個案例的highlight結構以支持兩個分離的區域
          {
            ...
            examples: [
              ...
              { label: 'OR (或)', latex: 'x < 2 \\text{ 或 } x > 5', result: 'x < 2 或 x > 5', color: 'red', highlight: { 
                regions: [
          ...
          }ive:scale-95 border border-red-300"
        >
          ⌫ 刪除
        </button>
        <button
